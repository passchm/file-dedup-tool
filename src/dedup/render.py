...
def main() -> None:
    ...
    style_text = (Path(__file__).parent / "resources" / "style.css").read_text()
    main_script_text = (Path(__file__).parent / "resources" / "main.js").read_text()

    html_tree = XHT.page(
        [
            XHT("style", {}, style_text),
            XHT("script", {"defer": "defer"}, main_script_text),
        ],
